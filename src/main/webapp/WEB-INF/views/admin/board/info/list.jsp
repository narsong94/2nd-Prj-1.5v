...
<table>
	...
	<tbody>
		<c:forEach var="i" items="${list}">
			<tr>
				<td>${i.id}</td>
				<td><a href="${ctx}/admin/board/info/${i.id}">${i.title}</a></td>
	...
	<a href="?p=${startNum != 1}">이전</a>
		...
		<c:forEach var="i" begin="0" end="4">	<!-- ${lastNum-1} -->
			...
			<c:if test="${startNum+i <= lastNum}">
				<li><a class="${strong}" href="${ctx}/admin/board/info/?p=${startNum+i}">${startNum+i}</a></li>
			</c:if>
...
<div>
	<a href="${ctx}/admin/board/info/reg">추가하기</a>
</div>
</main>