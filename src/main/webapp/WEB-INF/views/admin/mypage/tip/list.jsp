...
<main>
목록을 누르면 Tip 게시판에 해당 게시글로 이동됩니다.
<form method="get">
...
<table>
	...
	<tbody>
		<c:forEach var="t" items="${list}">
			<tr>
				<td>${t.id}</td>
				<td><a href="${ctx}/admin/mypage/tip/${t.id}">${t.title}</a></td>
				<td><fmt:formatDate pattern="yyyy-MM-dd" value="${t.date}" />
				</td>
				<td>${t.hit}</td>
				<td><input type="checkbox" name="tipChk" value="${t.id}"/></td>
			</tr>
		</c:forEach>
	</tbody>
</table>
...
<div>
	<a href="${ctx}/admin/mypage/tip/reg">게시글 등록</a>
	<a href="${ctx}/admin/board/tip">Tip 게시판으로</a>
</div>
</main>