...
<table>
	...
	<tbody>
		<c:forEach var="v" items="${list}">
			<tr>
				<td>${v.id}</td>
				<td><a href="${ctx}/admin/mypage/voting/${v.id}">${v.title}</a></td>
				<td><fmt:formatDate pattern="yyyy-MM-dd" value="${v.date}" />
				</td>
				<td>${v.hit}</td>
				<td><input type="checkbox" name="votingChk" value="${v.id}"/></td>
			</tr>
		</c:forEach>
	</tbody>
</table>
...
<div>
	<a href="${ctx}/admin/mypage/voting/reg">게시글 등록</a>
	<a href="${ctx}/admin/board/voting">선택 게시판으로</a>
</div>
</main>